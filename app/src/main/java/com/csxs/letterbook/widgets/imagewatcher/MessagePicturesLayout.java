...
public class MessagePicturesLayout extends FrameLayout implements View.OnClickListener {
    ...
    public MessagePicturesLayout(Context context, @Nullable AttributeSet attrs) {
        ...
        requestOptions = new RequestOptions()
                ...
                .error(R.drawable.ic_image_pholder_dynamic).transform(new CenterCrop(), new RoundBitmapTranformation(ScreenUtil.dip2px(context, 5)));
        DisplayMetrics mDisplayMetrics = context.getResources().getDisplayMetrics();
        mSingleMaxSize = (int) (TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 130, mDisplayMetrics) + 0.5f);
    ...
    private void notifyDataChanged() {
     ...
     //    int imageSize = 0;
//                = urlListSize == 1 ? mSingleMaxSize :
//                (int) ((getWidth() * 1f - mSpace * (column - 1)) / column);
        int imageSize = 0;
        if(urlListSize==1){
            imageSize=mSingleMaxSize;
        }else if(urlListSize==4){
            imageSize=   (int) (((getWidth()-getWidth()/3) * 1f - mSpace * (column - 1)) / column);
        }else{
...
//                iPicture.setBackgroundResource(R.drawable.default_picture);
              //  Glide.with(getContext()).load(thumbList.get(i)).transition(DrawableTransitionOptions.withCrossFade()).transform(new RoundBitmapTranformation(ScreenUtil.dip2px(getContext(), 5))).into(iPicture);
             //   ImageLoaderV4.getInstance().displayRoundImage(getContext(), thumbList.get(i).toString(), iPicture, R.drawable.default_picture, ScreenUtil.dip2px(getContext(), 5));
                Glide.with(getContext()).load(thumbList.get(i)).apply(requestOptions).into(iPicture);
