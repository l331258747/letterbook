...
public class NineImageView extends AppCompatImageView {
    ...
    public NineImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.NineImageView_android_src) {
            mDefaultImageId = typedArray.getResourceId(attr, 0);
        } else if (attr == R.styleable.NineImageView_bga_iv_circle) {
            mCircle = typedArray.getBoolean(attr, mCircle);
        } else if (attr == R.styleable.NineImageView_bga_iv_cornerRadius) {
            mCornerRadius = typedArray.getDimensionPixelSize(attr, mCornerRadius);
        } else if (attr == R.styleable.NineImageView_bga_iv_square) {
            mSquare = typedArray.getBoolean(attr, mSquare);
        } else if (attr == R.styleable.NineImageView_bga_iv_borderWidth) {
            mBorderWidth = typedArray.getDimensionPixelSize(attr, mBorderWidth);
        } else if (attr == R.styleable.NineImageView_bga_iv_borderColor) {
